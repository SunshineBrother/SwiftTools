...
final class NewLoggerPlugin: PluginType {
    ...
    func didReceive(_ result: Result<Response, MoyaError>, target: TargetType) {
        ...
        case .success(let response):
            ...
            print("  ┌────────────────────────────────────────────────────────────────\n")
            print("  ├┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄网络请求完成┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n")
            print("  ├ 请求类型: \(response.request?.httpMethod ?? "无") \n")
            print("  ├ 请求地址: \(response.request?.url?.absoluteString ?? "无") \n")
            print("  ├ 响应代码: \(response.statusCode) \n")
            print("  ├ 请求头部: \(response.request?.allHTTPHeaderFields ?? [:]) \n")
            print("  ├ Body数据: \(response.request?.httpBody?.stringValue ?? "无") \n")
            print("  ├ 请求耗时: \(String(format: "%.2f", time(for: target.path)))ms \n")
            print("  ├ 服务器返回数据:  \((try? response.mapString()) ?? "无") \n")
            print("  └──────────────────────────────────────────────────────────────── \n")
